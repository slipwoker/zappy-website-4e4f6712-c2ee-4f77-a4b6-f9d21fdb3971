document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (minLength && !VALIDATORS.minLength(value, minLength)) {
        setFieldError(field, ERROR_MESSAGES.minLength + ' (min ' + minLength + ' characters)');
        return false;
      }
  ...
  // ============================================================
  const ANIMATION_CLASSES = ['fade-in', 'fade-in-up', 'fade-in-down', 'fade-in-left', 'fade-in-right', 'slide-up', 'zoom-in'];
  const ANIMATED_CLASS = 'animated';
  const VISIBLE_CLASS = 'is-visible';

  function buildAnimationSelector() {
    return ANIMATION_CLASSES.map(function (cls) {
      return '.' + cls + ':not(.' + ANIMATED_CLASS + ')';
    }).join(', ');
  }

  function initScrollAnimations() {
    const selector = buildAnimationSelector();
      ...
      const observer = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            ...
            if (delay) {
              setTimeout(function () {
                el.classList.add(VISIBLE_CLASS, ANIMATED_CLASS);
              }, parseInt(delay, 10));
            } else {
              el.classList.add(VISIBLE_CLASS, ANIMATED_CLASS);
            }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Omer משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '4e4f6712-c2ee-4f77-a4b6-f9d21fdb3971',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '4e4f6712-c2ee-4f77-a4b6-f9d21fdb3971';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '4e4f6712-c2ee-4f77-a4b6-f9d21fdb3971';
